...
All actions in §3 above (nodes, flows, products, SKUs, swim-lanes, BOMs, demands, annotations).
...
(`update_flow` uses `{ ...fields }` — `bidirectional` is included automatically.)
...
The following is the reference text that `DDS_ACTIONS.getVocabularyText()` must produce. This text is injected into the Claude system prompt via the `{{ACTION_VOCABULARY}}` placeholder.

```
## Nodes
- add_node: required name; optional type_code, swim_lane_id, tags, notes
  Note: x, y are never accepted — canvas position is map-specific.
  Note: include "id": "new_node_N" when this node is referenced by subsequent actions.
- update_node: required node_id; optional name, type_code, swim_lane_id, tags, notes
- delete_node: required node_id
  Note: cascades to flows (source or target), SKUs, BOMs, BOM components, demands. List all explicitly.
- assign_node_to_lane: required node_id, swim_lane_id

## Flows
- add_flow: required source_id, target_id; optional lead_time_value, lead_time_unit, bidirectional, tags, notes
  Note: a flow with no products is valid.
  Note: include "id": "new_flow_N" when referenced by subsequent actions.
- update_flow: required flow_id; optional lead_time_value, lead_time_unit, bidirectional, tags, notes
- delete_flow: required flow_id
  Note: cascades to orphan SKUs and their BOMs and demands. List all explicitly.
- reroute_flow: required flow_id + at least one of new_source_id, new_target_id
- add_product_to_flow: required flow_id, product_id
- remove_product_from_flow: required flow_id, product_id

## Products
- add_product: required name; optional type_code, tags, notes
  Note: include "id": "new_product_N" when referenced by subsequent actions.
- update_product: required product_id; optional name, type_code, tags, notes
- delete_product: required product_id
  Note: cascades to flows, SKUs, demands. List all explicitly.

## SKUs
A SKU is a node × product association. Tags express the nature: buffer, stock, transit, etc.
- add_sku: required node_id, product_id; optional tags, notes
- update_sku: required node_id, product_id; optional tags, notes
- remove_sku: required node_id, product_id
  Note: cascades to demand for this SKU and its map_demands. List explicitly.

## Swim-lanes
- add_swim_lane: required name; optional color
  Note: include "id": "new_lane_N" when referenced by subsequent actions.
- update_swim_lane: required swim_lane_id; optional name, color
Note: swim-lane deletion is excluded from v1.

## BOMs
A BOM describes a node transformation: one output product from one or more input components.
- add_bom: required node_id, output_product_id; optional notes
  Note: include "id": "new_bom_N" when referenced by subsequent add_bom_component actions.
  Note: output product must exist as a SKU on the node.
- update_bom: required bom_id; optional output_product_id, notes
- delete_bom: required bom_id
  Note: cascades to all bom_components. List explicitly.
- add_bom_component: required bom_id, product_id, quantity; optional notes
  Note: component product must exist as a SKU on the node.
- update_bom_component: required bom_id, product_id; optional quantity, notes
- remove_bom_component: required bom_id, product_id

## Demands
A demand captures CTT and average demand per period for a SKU. At most one per SKU.
- CTT (Customer Tolerance Time): max time a customer waits without losing the sale.
  Used in DDMRP buffer positioning to determine where decoupling points are needed.
